...
Судя по описанию модуля `ec2-instance` у него практически все параметры соответствуют
ресурсу `aws_instance`. Получается, что преимуществ у `ec2-instance` перед `aws_instance` нет.
К сожалению не удается зарегистрироваться на AWS чтобы попробовать на практике модуль `ec2-instance`.
